...
from backend.services.hashing import compute_hash
...
async def intake_bundle(
    ...
    if req.audio_base64:
        ...
        if media_store.configured:
            ...
            # Dev fallback: spill bytes to local disk instead of dropping
            # them — intake:{id} refs are unplayable, but the audio stays
            # inspectable under data/generated/audio/.
            audio_ref = f"intake:{act.id}"
            try:
                from pathlib import Path as _P
                _spill = _P("data/generated/audio")
                _spill.mkdir(parents=True, exist_ok=True)
                (_spill / f"intake-{act.id}.wav").write_bytes(audio_bytes)
            except Exception:
                pass
...
async def get_performance_avatar(act_version_id: uuid.UUID):
    ...
    Never leaves the stage black: missing asset falls back to the
    default stage avatar (moves, no lipsync — see resolve_avatar)."""
    from fastapi.responses import RedirectResponse

    from backend.services.freaktown.bundle import resolve_avatar

    manifest = await get_performance(act_version_id)
    ref, source = resolve_avatar(manifest)
    if source == "sealed":
        if not media_store.configured:
            raise HTTPException(404, "media not configured")
        try:
            url = media_store.get_signed_url(ref)
        except Exception:

"""Procedural walkout synth — the instant engine.

Ported from freaktown sound_synth.py (Black Room): genre + mood + energy
+ shape in, exact WAV out. Deterministic per (recipe, seed). Stdlib only —
no GPU, no API, no network.

This is NOT a placeholder for Stable Audio. Instant deterministic
feedback beats slow rich generation for the creation loop: roll a Freak,
immediately hear *something*. MAGIC REROLL (model backends) stays
available above this in the provider chain.

Chain: remote worker → local GPU (Stable Audio) → procedural (always).
"""
...
# Killella-only genres fall back to the nearest procedural kit.
GENRE_FALLBACK = {
    "metal": "rock",
    "punk": "rock",
    "country": "funk",
...
def _osc(wave_type: str, freq: float, t: float) -> float:
    ...
    return 2 * ((freq * t) % 1) - 1  # saw


def resolve_genre(genre: str) -> tuple[str, dict]:
    """Map any factory genre to a procedural kit. Returns (name, kit)."""
    if genre in GENRES:
        return genre, GENRES[genre]
    fallback = GENRE_FALLBACK.get(genre, "funk")
    return fallback, GENRES[fallback]


def generate_wav(recipe: dict, seed: int = 0) -> bytes:
    """Generate a walkout WAV. recipe: {genre, mood, energy, shape, duration}."""
    genre = recipe.get("genre", "funk") if isinstance(recipe, dict) else "funk"
    _, g = resolve_genre(genre)
...
def recipe_id(recipe: dict, seed: int = 0) -> str:
    canon = json.dumps({**recipe, "seed": seed}, sort_keys=True)
    return hashlib.sha256(canon.encode()).hexdigest()[:12]


class ProceduralProvider:
    """Always-available local synth. Instant, deterministic, free."""

    def is_available(self) -> bool:
        return True

    async def generate(self, recipe) -> "AudioAsset":
        from backend.services.audio.factory import AudioAsset, build_walkout_prompt

        params = {
            "genre": recipe.genre, "mood": recipe.mood, "energy": recipe.energy,
            "shape": recipe.shape, "duration": min(recipe.duration, 11.0),
        }
        wav = await asyncio.to_thread(generate_wav, params, recipe.seed)
        prompt = f"procedural:{recipe.genre}/{recipe.mood}/{recipe.energy}/{recipe.shape}"
        try:
            prompt = build_walkout_prompt(recipe)
        except Exception:
            pass
        return AudioAsset(
            asset_id=recipe.asset_id, audio_bytes=wav,
            duration=params["duration"], recipe=recipe.to_dict(), prompt=prompt,
        )
